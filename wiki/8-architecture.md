...
5. If the model tries a **write** tool (anything that changes data in an external app), the call is intercepted, a row is created in `kasie_pending_actions`, and the run moves to `awaiting_approval`. This is HITL: human-in-the-loop, meaning a person approves before the agent acts. Approving in Slack (`/api/slack/interactions`) executes the tool and completes the run.
